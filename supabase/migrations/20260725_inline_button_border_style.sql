-- Inline try-on button border treatment: 'halo' | 'gradient' | 'double' |
-- 'shimmer'; NULL = none. Picked in the dashboard (widget-design), applied by
-- widget-main (elloApplyInlineBtnBorder) to the theme block's
-- .ello-inline-tryon-btn — widget-side so it ships via Cloud Run, not the
-- theme-extension pipeline.
--
-- APPLIED TO PROD via Supabase MCP 2026-07-25 (grants verified identical
-- post-recreate). Def = live prod def + inline_button_border_style only.

ALTER TABLE public.vto_stores
  ADD COLUMN IF NOT EXISTS inline_button_border_style text
  CHECK (inline_button_border_style IS NULL OR inline_button_border_style IN ('halo','gradient','double','shimmer'));
